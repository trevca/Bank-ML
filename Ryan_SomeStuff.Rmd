...
```{r}
ggplot(bank_data,aes(job))+geom_bar()
...
```{r}
n=nrow(bank_data)
ntree=2500 #Number of Trees
tsize=7500 #size of test set
sam=sample(n,tsize)
test=bank_data[sam,]
train=bank_data[-sam,]
```
...
summary(boost)
```
From looking at the important table and graph, it is clear that the euribor3m is the strongest predictor. As we saw above, it should be noted that this variable was strongly correlaated with the "employment variation rate" and "number of employees." All of these are metrics of the economy and are likely also correlated with the interest rates the bank is offering. "Month" was a distant but clear second; this is of interest as nothing immediately comes to mind for why month would have a strong effect. One possibility is that given that the data was gathered over the short span of a year and a half, outside events could have had an effect on some months but there was not enough time for those outside unrelated events to get averaged out. The third most important predictor was the persons "job," which makes sense logically since wealthier people will likely have more money to invest. Following job, no other variables make larger contributions, although they do have small effects.

...
```{r}
bank_data=read_delim("bank-additional/bank-additional.csv",";",col_names=TRUE,col_types = "dfffffffffddddfdddddf")#,col_types = "dffccffffcddddfdddddf")
```
...
eval<-function(shrink,pp){
  ...
  for (i in 0:4){
    ...
    val<<-train[kfold,]
    pval<<-pp
    tr=ovun.sample(y~.,tr,method="over",p = pval)$data
...
x=BayesianOptimization(eval, bounds = list(shrink=c(.0001,.1),pp=c(.3,.7)), acq = "ucb",n_iter=10,init_points=5)
train=ovun.sample(y~.,train,method="over",p=x$Best_Par[2])$data
boost=gbm(y~.,data=train,distribution="bernoulli",n.trees=1000, shrinkage=x$Best_Par[1])
summary(boost)
